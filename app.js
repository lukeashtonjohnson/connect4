...
// step 3
  ...
  function checkBoard() {
    for (let y = 0; y < winningArrays.length; y++) {
      ...
      //check those square to see if they all have the class of player-one
      if (
  ...
  for (let i = 0; i < square.length; i++) {
    square[i].addEventListener('click',() => {
      ...
      // add 7 is selects the spot above where you have just clicked and checked to see if it has been taken and 
      if (square[i + 7].classList.contains('taken') ) { // &&!square[i].classList.contains('taken')
        if (currentPlayer == 1) {
      ...
      } else alert('You cant go here, try again')
      checkBoard();
    });
  }
